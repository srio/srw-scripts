...
def srwArrays_to_numpyArrays(ar2d_input, x_range, y_range):

  totLen = int(x_range[2]*y_range[2])
  ar2d_output = numpy.zeros(totLen)
  lenAr2d = len(ar2d_input)

  if lenAr2d > totLen:
    print("REDUCING....")
    ar2d_output[0:totLen] = np.array(ar2d_input[0:totLen]).copy()
  elif lenAr2d < totLen:
    print("PADDING....")
    ar2d_output[0:lenAr2d] = np.array(ar2d_input[0:lenAr2d]).copy()
  else:
    print("SAME...")
    ar2d_output = np.array(ar2d_input)

  ar2d_output.shape = (y_range[2],x_range[2])

  x = np.linspace(x_range[0],x_range[1],x_range[2])
  y = np.linspace(y_range[0],y_range[1],y_range[2])

  return ar2d_output.T,x,y
...
def main():
  ...
  arI0_1 = numpy.array(arI0).copy()
  arI0_2 = numpy.array(arI0).copy()

  #
  # old plot
  #
  # uti_plot.uti_plot2d(arI0_1, plotMesh0x, plotMesh0y, ['Horizontal Position [mm]', 'Vertical Position [mm]', 'Intensity Before Propagation'])


  #
  # new plot
  #
  arI0_np,x_np,y_np = srwArrays_to_numpyArrays(arI0_2,plotMesh0x,plotMesh0y)
  plot_image(arI0_np,x_np,y_np,title='Intensity at %4.1f m '%zStart,xtitle='Horizontal Position [mm]',ytitle='Vertical Position [mm]')
